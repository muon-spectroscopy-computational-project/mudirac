...
vector<double> energyCorrections(int Z, int n1, int l1, bool s1, int n2, int l2, bool s2)
{
    vector<double> E_corr;
    DiracAtom da_point = DiracAtom(Z, Physical::m_mu, getIsotopeMass(Z));
    DiracAtom da_sphere = DiracAtom(Z, Physical::m_mu, getIsotopeMass(Z), SPHERE);
    DiracAtom da_uehling = DiracAtom(Z, Physical::m_mu, getIsotopeMass(Z), SPHERE);
...
{
    AixLog::Log::init<AixLog::SinkCout>(AixLog::Severity::warning, AixLog::Type::all);
    SECTION("12Mg")
    {
        // 3d3/2-2p1/2
