...
void writeFermiParameters(DiracAtom &da, const double opt_time, string fname, int output_precision) {
  ...
  out << "fermi_c\tfermi_t\trms_radius\treduced_chi2\ttime\tsigma_c\tsigma_t\tsigma_rms_radius\tcov_c_t\n";
  out << fixed;
  out << setprecision(output_precision);

  // output fermi_c, fermi_t, rms radius, reduced chi2, optimisation time and uncertainties
  out << da.fermi2.c << '\t' << da.fermi2.t << '\t';
  out << da.fermi2.rms_radius << '\t';
  out << scientific;
