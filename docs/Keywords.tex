...
\author{Simone Sturniolo, Leandro Liborio, Milan Kumar, Subindev Devadasan}
...
\texttt{mudirac} takes a main input file, and optionally a second experimental results file when running Fermi parameter optimisation:

\begin{verbatim}
mudirac main.in [experimental.in]
\end{verbatim}

The main input file contains multiple lines with the format
...
\begin{itemize}
	...
	\item \texttt{2pF\_coords}: coordinate system to use for the 2-parameter Fermi (2pF) optimisation. Can be CT (half-density radius $c$ and skin thickness $t$) or POLAR (rms radius and polar angle $\theta$, where $\tan\theta = t/c$). The POLAR parametrisation gives better-conditioned optimisation and is recommended for most nuclei; CT should be used when $A < 5$. Only relevant when \texttt{optimise\_fermi\_parameters = TRUE}. Default is POLAR.
	\item \texttt{min\_2pF\_algorithm}: minimisation algorithm for Fermi parameter optimisation. Can be \texttt{lm} (Levenberg--Marquardt, recommended) or \texttt{ls} (L-BFGS line search). Only relevant when \texttt{optimise\_fermi\_parameters = TRUE}. Default is \texttt{lm}.
...
\begin{itemize}
	\item \texttt{uehling\_correction}: whether to turn on or not the Uehling correction (vacuum polarisation). Recommended when running Fermi parameter optimisation against experimental data. Default is FALSE.
	\item \texttt{optimise\_fermi\_parameters}: if true, optimises $c$ and $t$ from the 2pF model. If false, no optimisation is performed and $c$ and $t$ can be set manually. Requires a second experimental results file: \texttt{mudirac main.in experimental.in}. Default is FALSE.
	\item \texttt{reduced\_mass}: whether to turn on the non-relativistic recoil correction or not. If true, the reduced mass of the muon is calculated and applied using the nuclear mass and the muon mass (defined by the \texttt{mass} keyword if provided). Default is TRUE.
	\item \texttt{write\_spec}: if true, write a spectrum file using the transition lines found broadened with Gaussian functions. Other keywords starting with \texttt{spec\_} can then be specified. Default is FALSE.
	\item \texttt{sort\_by\_energy}: if true, print out the transitions sorted by energy instead of by shell. Default is FALSE.
\end{itemize}
...
\begin{itemize}
	...
	\item \texttt{radius}: solid sphere equivalent radius ($r_\text{rms} \times \sqrt{5/3}$). By default it is set to values from Angeli and Marinova (2013).
	\item \texttt{fermi\_t}: skin thickness for a Fermi model nucleus. By default it is 2.3 fm.
	\item \texttt{fermi\_c}: Fermi parameter $c$. By default $c = \sqrt{R^2 - \frac{7}{3}\left(\frac{\pi T}{4\ln 3}\right)^2}$ for $A \geq 5$, or $c = 2.2291 \times 10^{-5} \cdot A^{1/3} - 0.90676 \times 10^{-5}$ for $A < 5$.
	...
	\item \texttt{loggrid\_center}: center of the logarithmic grid in units of $a_0 = 1/(Zm)$. Default is 1.
...
\begin{itemize}
	...
	\item \texttt{xr\_print\_precision}: number of digits after the decimal point for energies and rates in the \texttt{.xr.out} file. Default is -1 (maximum precision).
	\item \texttt{state\_print\_precision}: number of digits after the decimal point in Dirac state output files. Default is -1 (maximum precision). Only has effect if \texttt{output} $\geq$ 2.
	\item \texttt{rms\_radius\_decimals}: number of decimal places to use for the rms radius in the \texttt{.fermi\_parameters.out} output file. Only relevant when \texttt{optimise\_fermi\_parameters = TRUE}. Default is 2.
...
\section{Experimental results file keywords}\label{APP:expkeyw}

When \texttt{optimise\_fermi\_parameters = TRUE}, a second input file must be provided containing the experimental X-ray transition data. This file uses the same \texttt{keyword: value} format and accepts the following keywords.

\subsection{String keywords}

\begin{itemize}
	\item \texttt{xr\_lines}: the transitions for which experimental energies are provided, using the same IUPAC notation as in the main input file. The list must match the \texttt{xr\_lines} in the main input. Multiple lines separated by commas.
\end{itemize}

\subsection{Floating point keywords}

\begin{itemize}
	\item \texttt{xr\_energy}: experimental transition energies in eV, comma-separated, in the same order as \texttt{xr\_lines}.
	\item \texttt{xr\_error}: 1$\sigma$ uncertainties on the experimental energies in eV, comma-separated, in the same order as \texttt{xr\_lines}. Used as weights in the $\chi^2$ minimisation.
\end{itemize}

An example experimental results file for $^{206}$Pb:

\begin{verbatim}
xr_lines:  K1-L2, K1-L3, L2-M4, L3-M5
xr_energy: 5788330, 5973980, 2643750, 2501450
xr_error:  480, 440, 360, 430
\end{verbatim}
